...
const CustomersTable = (props:any) => {
  ...
  // Prevent event propagation when clicking checkboxes
            ...
            <tbody className="px-3">
              {customer?.data?.customers?.map((cust:any) => (
                ...
                >
                  ...
                  <td className="text-[12px] font-[300] py-4 whitespace-nowrap">{cust.first_name} {cust.last_name}</td>
                  <td className="text-[12px] font-[300] py-4 whitespace-nowrap">{cust.phone_number}</td>
                  <td className="text-[12px] font-[300] py-4 whitespace-nowrap">{cust.email}</td>
                  <td className="text-[12px] font-[300] py-4 whitespace-nowrap">{cust.username}</td>
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  <div className="flex items-center gap-3">
                    <input
                      type="checkbox"
                      id={`mobile-checkbox-${cust.id}`}
                      name="customCheckbox"
                      className="w-4 h-4 text-primary bg-gray-100 border-gray-300 rounded focus:ring-blue-500 focus:ring-2"
                      onClick={handleCheckboxClick}
                    />
                    <h3 className="font-medium text-gray-900">{cust.first_name} {cust.last_name}</h3>
                  </div>
                  <span
                    className="text-[10px] font-[500] rounded-[10px] py-[2px] px-[10px]"
                    style={{
                ...
                <div className="grid grid-cols-2 gap-2 text-[12px] text-gray-500">
                  ...
                  <div>{cust.phone_number || "N/A"}</div>
                  
                  <div className="font-medium">Email:</div>
                  <div className="truncate">{cust.email || "N/A"}</div>
                  
                  <div className="font-medium">Username:</div>
                  <div>{cust.username || "N/A"}</div>
                </div>
              </div>
            ))}
          </div>
